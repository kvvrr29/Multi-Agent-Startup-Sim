...
// ~828 MiB to download once, ~1630MB of VRAM.
    ...
    try {
        ...
        this.modelId,
        {
          initProgressCallback: (info) => {
            ...
            if (import.meta.env.DEV && !hasCached) {
              console.log(
                `[Diagnostic Download] Progress: ${Math.round(info.progress * 100)}% | ${info.text}`,
              );
            }
            this._notify();
          },
      ...
      logDiagnostic("ERRORS", {
        ...
        "User-friendly UI message":
          "Built-in AI is unavailable on this browser.",
      });
    ...
    try {
      ...
      if (this.engine) {
        try {
          ...
          console.warn(
            "[ModelManager] Engine unload failed; continuing with cache delete:",
            err,
          );
        }
      }
      ...
      this.status = "error";
      this.progress = {
        text: `Could not remove the model: ${err.message}`,
        progress: 0,
      };
      this._notify();
      throw err;
