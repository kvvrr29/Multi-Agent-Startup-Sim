...
import {
  useProjectResourceStore,
  PROJECT_RESOURCES,
} from "../store/useProjectResourceStore";
...
const sectionFingerprint = (section = {}) =>
  JSON.stringify({
...
const metaFingerprint = (project) =>
  JSON.stringify({
    name: project?.name || "Untitled Project",
  });

const memoryFingerprints = (memory = {}) =>
  Object.fromEntries(
    MEMORY_CATEGORIES.map((category) => [
      category,
      Object.fromEntries(
        Object.entries(memory[category] || {}).map(([key, value]) => [
          key,
...
const snapshotCursor = () => {
  ...
  return {
    sectionsByKey: Object.fromEntries(
      Object.entries(blueprint || {}).map(([key, section]) => [
        key,
        sectionFingerprint(section),
      ]),
    ),
...
const doPush = async () => {
  ...
  const changedSections = Object.entries(blueprint || {})
    .filter(
      ([key, section]) =>
        ...
        next.sectionsByKey[key] !== cursor.sectionsByKey[key],
    )
    .map(([key, section]) => ({
      key,
      content: section.content,
  ...
  if (changedSections.length > 0) {
    tasks.push({
      name: "sections",
      run: () => api.upsertSections(activeCloudId, changedSections),
      commit: () => {
        changedSections.forEach((s) => {
          cursor.sectionsByKey[s.key] = next.sectionsByKey[s.key];
        });
      },
    });
  }
  ...
  if (newEvents.length > 0) {
    tasks.push({
      name: "events",
      run: () => api.appendEvents(activeCloudId, cloneSerializable(newEvents)),
      commit: () => {
        cursor.eventCount = next.eventCount;
      },
    });
  }
  ...
  if (newDecisions.length > 0) {
    tasks.push({
      name: "decisions",
      run: () =>
        api.appendDecisions(activeCloudId, cloneSerializable(newDecisions)),
      commit: () => {
        cursor.decisionCount = next.decisionCount;
      },
    });
  }
  ...
  const changedMemoryEntries = MEMORY_CATEGORIES.flatMap((category) =>
    Object.entries(memory[category] || {})
      .filter(
        ([key]) =>
          next.memoryByKey[category]?.[key] !==
          cursor.memoryByKey[category]?.[key],
      )
  ...
  if (changedMemoryEntries.length > 0) {
    tasks.push({
      name: "memory",
      run: () =>
        api.upsertMemory(activeCloudId, { entries: changedMemoryEntries }),
      commit: () => {
  ...
  if (next.metaJSON !== cursor.metaJSON) {
    tasks.push({
      name: "meta",
      run: () =>
        api.updateProjectMeta(activeCloudId, {
          name: project?.name || "Untitled Project",
        }),
      commit: () => {
        cursor.metaJSON = next.metaJSON;
      },
    });
  }
  ...
  results.forEach((result, i) => {
    if (result.status === "fulfilled") {
      ...
      allOk = false;
      console.error(
        `[Cloud] Sync failed for ${tasks[i].name}:`,
...
export const startSync = () => {
  unsubscribeAll();
  unsubscribers = [useProjectStore, useProjectMemoryStore].map((store) =>
...
export const resetProjectResourceLoading = (
  projectId = null,
  status = "idle",
) => {
  resourceEpoch += 1;
...
export const createCloudProject = async (form) => {
  const { session, setActiveCloudId, addCloudProject } =
...
export const openCloudProject = async (id) => {
  const registryProject = useAuthStore
    .getState()
  ...
  if (useAuthStore.getState().activeCloudId === id) await pendingFlush;

  let data;
  try {
    [, data] = await Promise.all([pendingFlush, api.getProjectBlueprint(id)]);
  } catch (err) {
...
const normalizeListPayload = (payload, key) =>
...
const buildMemory = (entries) => {
  const hydrated = Object.fromEntries(
    MEMORY_CATEGORIES.map((category) => [category, {}]),
  );
...
const applyHydratedResource = (resource, payload) => {
  ...
  try {
    if (resource === "meta") {
      ...
      const metaWasLocallyChanged =
        cursor && metaFingerprint(currentProject) !== cursor.metaJSON;
    ...
    if (resource === "events") {
      ...
      const localEvents = useProjectStore
        .getState()
        .workflowEvents.slice(cursor?.eventCount || 0);
      useProjectStore.setState({
        workflowEvents: mergeAppendOnly(databaseEvents, localEvents),
      });
    ...
    if (resource === "memory") {
      const databaseMemory = buildMemory(
        normalizeListPayload(payload, "entries"),
      );
      ...
      const locallyChangedEntries = cursor
        ? MEMORY_CATEGORIES.flatMap((category) =>
            Object.entries(currentMemory[category] || {})
              .filter(
                ([key]) =>
                  currentFingerprints[category]?.[key] !==
                  cursor.memoryByKey[category]?.[key],
              )
              .map(([key, value]) => ({ category, key, value })),
          )
    ...
    if (resource === "decisions") {
      ...
      const localDecisions = useProjectMemoryStore
        .getState()
        .decisionHistory.slice(cursor?.decisionCount || 0);
...
const ensureOneResource = (projectId, resource, epoch) => {
  ...
  if (
    ...
    state.resources[resource]?.status === "ready"
  ) {
  ...
  const request = RESOURCE_LOADERS[resource](projectId)
    .then((payload) => {
      ...
      useProjectResourceStore
        .getState()
        .setResourceState(projectId, resource, "ready");
    })
    .catch((error) => {
      const isCurrent =
        epoch === resourceEpoch &&
...
export const ensureProjectResources = async (resourceNames) => {
  ...
  const resources = [...new Set(resourceNames)].filter((name) =>
    PROJECT_RESOURCES.includes(name),
  );
  const epoch = resourceEpoch;
  const results = await Promise.allSettled(
    resources.map((resource) => ensureOneResource(projectId, resource, epoch)),
  );
