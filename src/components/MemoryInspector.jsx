...
export default function MemoryInspector() {
  ...
  return (
    <div className="glass-panel" style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      ...
      {expanded && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '0.5rem', fontSize: '0.8rem', color: 'var(--text-muted)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px' }}>
            <div><strong>Domain:</strong> <span style={{ color: 'var(--text-primary)' }}>{memory?.scope?.domain || 'N/A'}</span></div>
            <div><strong>Industry:</strong> <span style={{ color: 'var(--text-primary)' }}>{memory?.scope?.industry || 'N/A'}</span></div>
          ...
          {Object.entries(memory).filter(([key]) => key !== 'domain').map(([category, items]) => {
            const keys = Object.keys(items || {}).filter(k => 
              !(category === 'scope' && ['domain', 'industry', 'project_type', 'business_model', 'complexity', 'mandatory_entities', 'confidence', 'reasoning'].includes(k))
            );
            if (keys.length === 0) return null;
            return (
              <div key={category} style={{ background: 'rgba(0,0,0,0.2)', padding: '6px 8px', borderRadius: '4px' }}>
                <strong style={{ textTransform: 'capitalize', color: 'var(--accent-cyan)' }}>{category}</strong>
                <ul style={{ margin: '4px 0 0 0', paddingLeft: '16px' }}>
                  {keys.map(k => (
                    <li key={k}><span style={{ color: 'var(--text-secondary)' }}>{k}:</span> <span style={{ color: 'var(--text-primary)' }}>{items[k]}</span></li>
                  ))}
                </ul>
              </div>
            );
